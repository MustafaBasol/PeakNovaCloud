...
import { getServices } from '@/libs/utils';
import { useLocale } from 'next-intl';
import PopUp from '@/components/dashboard/PopUp';
import { createServices, updateServices, deleteServices } from '@/libs/postUtils';
...
export default function ManageServices() {
  ...
  const addItem = () => {
    setCurrentItem({ title: '', description: '', language: locale, service: '', section:'', image:'',buttonText:'', cards:'' })
    setIsOpen(true)
    setError('')
  }
  ...
  const handleSubmit = async (e, formData) => {
    ...
    try {
      if(formData._id) {
        await updateServices(locale, formData)
      } else {
        await createServices(locale, formData)
      }
          ...
          services.map((item, i) => (
            ...
            >
              <div className="flex flex-col">
                <h6 className="font-bold">{item.service}</h6>
